...
| 尺寸 | 侧栏 224 · 顶栏 64 · 右面板 320 · 审核列表列 376 · 导航项 38 / 子项 32 · 分段控件 32 · 描边控件 34–36 · 主按钮 42 · 徽标 22–24 |
...
- 顶栏:白底,高 64px,下边框 `#EEF0F4`;左边页标题(19/700),需要时接一串上下文
  (`·` 小圆点分隔 + mono job type + 状态徽标 + 一句描述);右边全局动作用 34px 描边 pill
- 中间内容区:白底,内距 24/28
...
- **对话气泡**:用户消息 navy 底白字右对齐;助手消息白底左对齐;引用标 `[1]` 用知识类型识别色的浅底 pill
...
- 表单:label 在上(12.5/600 `foreground`);**输入框是填充块** —— `subtle` 底、透明边、12px 圆角、
  内距 13/16;聚焦 → 白底 + `ring` 边 + `--shadow-focus` 外圈;helper 11.5 `faint` 在下方
