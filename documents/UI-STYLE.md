...
**知识类型的固定识别色**(全站一致:导航点、KB 卡片、引用标记):
精准 QA = `#FFCB02`、文档 RAG = `#5B8DEF`、智能问数 = `#A78BFA`、
**编排(workflow)= `#00205B`(品牌 navy,token `--kb-workflow`)**。
编排是第四种知识(2026-08-27 加,只有设计预览没有后端):它**编排**另外三种,
所以刻意不引新色相,借用品牌 navy —— 与黄/蓝/紫都拉得开。
...
### 演示页字阶(唯一例外,作用域仅 `/how-it-works`)

上一条「不用 24px 以上」是工作台的规矩。`/how-it-works` 说明页不是工作台,它是**投屏讲稿**:
面试时共享屏幕讲设计思想,行距与字号都得比业务页大一档,否则远处读不清。所以给它开一处例外,
**范围严格限于 `/how-it-works` 及其子页**,其余任何页面不得引用本表。

| 用途 | 字阶 |
| --- | --- |
| 主张句(总页开场) | Montserrat 700 / 34px / `-.02em` |
| 屏标题 | Montserrat 700 / 30px / `-.02em` |
| 折叠区标题(总页目录行) | Montserrat 700 / 24px / `-.02em`,配 13px `faint` 关键词摘要 |
| 段标题 | Montserrat 600 / 20px |
| 正文 | Inter 400 / 17px / 1.65 |
| 强调句(每屏至多一句) | Inter 600 / 19px |
| 标注 / meta | Inter 400 / 13px `faint`(数字仍走 mono) |

版式同样只在这一页放宽:内容最大宽 **860px**(业务页 680px),右侧面板不挂载;
窄于 960px 时所有多列网格降为单列。这一组页面共两种(2026-08-27,v4「图先于字」):
**总页**「主张 + **全局图** → 路由漏斗 → 三层卡片 → 一次请求的判定流程图 → 四条立场」常驻,
其余九区**默认折叠**(架构五小节 + 原四区)—— 折叠态一行 = 24px 标题 + 13px 结论句摘要,
九行拼起来就是目录,顶部一个 Expand all / Collapse all 供投屏讲解一键全展开;
**三个层子页**同一模板。
架构图一律用填充块 + lucide 箭头画(不用内联 SVG:`<text>` 不换行,窄屏必溢出);
**每张图都要看得见方向** —— 步骤间有箭头、分岔左右分开(命中往右出 / 未命中往下走)、
闭环有虚线回边。**层级关系靠版面表达**:同级的东西并排等宽放在同一个框里,
兜底的东西必须在下一行、且由一条带原因的箭头引下去(不能把同级画成台阶)。
黄色在这一组页面里只有一个含义 —— Exact Q&A 的识别色,以及**人审闸门**(GATE 徽标 / 治理步),
不作他用。

除字阶与上述版式外**其余规范一律照旧**:颜色只用既有 token(不新增 hex)、圆角与阴影照既有档、
过渡仍是 150ms 一档、图标仍是 lucide 1.75、不用 emoji、不做渐变。
**实现纪律**:这套字阶只在 `web/src/pages/how-it-works/Section.tsx` 一处落地,
其他组件里出现 24px 以上的 sans 字号即视为违规。
