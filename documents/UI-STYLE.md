...
窄于 960px 时所有多列网格降为单列。这一组页面共两种(2026-08-26):
**总页**「主张 + 四条立场 + 架构段(六小节常驻,顶部一条锚点 chip 当目录)+ 三层卡片常驻,
其余四区**默认折叠**」—— 折叠态一行 = 标题 + 关键词摘要,当目录用,讲哪点开哪;
架构小节标题走 20px 段标题,与 30px 屏标题拉开层级;**三个层子页**同一模板。
架构图一律用填充块画(不用内联 SVG:`<text>` 不换行,窄屏必溢出)。
