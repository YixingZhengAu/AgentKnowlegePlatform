...
## 14. v5:层级纠正 + 第四种知识(2026-08-27)

需求方读完 v4 后指出两件事,一件是错、一件是缺:

### 14.1 错:v4 那张漏斗把同级画成了台阶

`FUNNEL` 把「精准问答 → 智能问数 → 文档 → 无依据」画成四级递进,读者会得出
「文档 RAG 是第三档知识」这个结论。**真实层级是两级**:

- **第一级 · 已注册的意图(三家同级)**:精准问答 / 智能问数 / **编排**。三者都在治理期
  写好并注册了自己的意图,回答时做**一次**意图匹配,谁命中谁执行,另外两家不跑。
  同级之间只有「谁更具体」的 tie-break —— 都命中时编排赢过一句干答案。
  (实现上这个 tie-break 落成 `core/chat.py` 里的固定顺序,所以代码不用改。)
- **兜底 · 开放检索**:文档 RAG,**只在没有任何意图命中时**才跑。再往下才是「说没有依据」。

落地:

- `FUNNEL` → `ROUTING`(`content.ts`),`FunnelFigure` → `RoutingFigure`(`figures.tsx`):
  三家**并排等宽**放在同一个框里(同级靠版面表达),往下一条带原因的箭头引到兜底,
  再一条到「无依据」;框内一句 `peerNote` 说清 tie-break。
- `REQUEST_PATH` 第 2 步改成**一步三出口**(`RequestStep.exits`):意图匹配那一格右边
  三个绿色出口块并列(Exact Q&A / Text-to-SQL / Workflow),未命中沿主干往下到兜底。
- `COMPARISON` 改成**四列**,列序 = 三家同级 + 兜底,并加一行「When it runs」把层级写进表里;
  表头色点改用 `COMPARISON.dots`(不再按下标去 `LAYERS` 里取,那会错位)。
- `LAYERS` 顺序改成 精准问答 → 智能问数 → 文档(侧栏子项与卡片区一起变),
  卡片顺序由 `KIND_CARD_ORDER` 给:两家意图层 → 编排 → 文档兜底。
- 连带改口径:`SYSTEM_MAP`(回答期那步 + 知识中枢多一块「Published workflows」)、
  `CLAIM.corollaries` R3、`STACK`(runtime 多一块 Workflow execution,control plane 多一道
  Action gate)、`AUTONOMY`(动作节点不许自己按)、`EVALUATION`(轨迹层多一条检查)、
  `PAIN_POINTS`(多一行「答完了,活还没干完」)、`GATE`(标题改成四种知识共用一道闸)。

### 14.2 缺:第四种知识 —— 编排(workflow)

**它是什么**:不产生新事实的知识。它把前三种按业务签过字的顺序连起来,外加代码节点
(阈值 / 分支,故意做成确定性的)与动作节点(写库 / 外发),从而**把一件事做完**,
而不是回答一句话。它也是一种意图 —— 所以在层级上与前两家同级(见 14.1)。

**四条纪律(页面上的 W1–W4)**:

| # | 纪律 | 为什么 |
| --- | --- | --- |
| W1 | 只能引用**已发布**的知识,引名字不引副本 | 编排不能凭空制造权威;缺知识就先去治理 |
| W2 | 参数**自动绑定** | 每个节点声明自己要什么,模型从前面几步里认出来(邮件里的订单号、查询返回的延期天数),不手工接线 |
| W3 | 编排本身**是一个意图** | 注册它服务的问法,命中就执行 —— 同级,不是更深一层 |
| W4 | **闸门不移动** | 读 / 判断 / 起草随便;写库、外发、改记录一律「准备好,人按下去」 |

**页面落点**:**自己的一页** `/how-it-works/workflow`(`WorkflowPage`,侧栏里和三层并列;
见 §14.4 —— 初版是总页最后一节,需求方要求把它抓成并列的 section)—— 概念图
(已发布知识 → `referenced by name, never copied` → 四种节点)+ W1–W4 + 一句
「已设计、未落地」+ 那条客服邮件编排**逐节点**的 in / out / 绑定参数(八个节点:
触发 → 读意图与情绪(LLM)→ 查订单(问数)→ 判延期档(代码)→ 该档的处理口径(精准问答)
→ 该档的邮件模板(精准问答)→ 写回信(LLM)→ 记录并改期(动作,人审))。
卡片区第四张卡链到它。

**摄取侧**:`web/src/domains/workflow/` —— 导航里占个位置 + 一页静态画布预览
(面板 / 画布 / 检查器三栏,横幅写明不可交互、不落库、不发请求),**不做开发**。
识别色新增 token `--kb-workflow`(品牌 navy,`index.css`;UI-STYLE §2 已同步)。

### 14.3 纪律

- 仍然不用内联 SVG、不引图表库;新增的唯一色值是 `--kb-workflow`,仍然只写在 `index.css`。
- **层级关系一律靠版面表达**:同级并排等宽同框,兜底另起一行并由带原因的箭头引下去。
- 编排那一节必须自己说清「已设计、未落地」,并且动作节点上明着标 GATE ——
  和 `AUTONOMY` 那段(今天这个 agent 不做任何动作)不能互相打脸。
- 实测:1440px / 1000px 两档无横向滚动;四张卡片窄屏降成单列,占位页三栏降成竖排。

### 14.4 v5.1 收口(同日,需求方两条)

1. **编排从「总页收尾一节」升成一页**:`WorkflowPage`(`/how-it-works/workflow`),
   侧栏 How It Works 分组里和三层**并列**四页,顺序 = `KIND_CARD_ORDER`
   (精准问答 → Text-to-SQL → 编排 → 文档兜底,和页内层级同一口径)。
   `HOW_IT_WORKS_NAV` 因此改成由 `KIND_CARD_ORDER` 生成,不再只遍历 `LAYERS`;
   路由里 `/how-it-works/workflow` 写在 `:layer` 之前(它不是 `LayerSlug`)。
   页内锚点 `#workflows` 随之作废,`domains/workflow` 的返回链接改指新路由。
2. **署名前置**:`AUTHORS` 从页脚挪到总页**页首**(一条横排:label + 两个名字 + 一句注),
   `note` 里的「above」改「below」。理由:面试场景先说谁做的,再进主张。
