...
> **演示知识的迁移**:`scripts/fixtures/s3/`(语义层 / 意图的模板与参数区 / 相似问法 /
...
>
> **后来加过意图(2026-08-24)**:为了让演示台账不至于太空,又手写了 5 条意图
> `i19`(按州拆收入)/ `i20`(销售代表榜)/ `i21`(各仓当前库存)/ `i22`(产品目录与标价)/
> `i23`(按渠道看单量与客单价),模板 SQL 逐条拿真库跑过。上面那些"7 个意图 / 75 条索引面"
> 是 B 阶段的数字,现在是 12 个意图。**选题刻意避开评测集里那 4 条"必须拒答"的越界问题**
> (按产品品类拆收入 / 利润率 / 客户电话与信用额度 / 按品类过滤出库)—— 给它们配上模板就等于
> 自己把那道硬闸门拆了。加完复跑评测集仍 20/20、硬闸门仍 7/7(证据见 `documents/DEPLOY.md` §6.5)。
...
## 收尾后的修补(S3 闭环之后发现的问题,逐条记账)

### 1. 模板 SQL 落库时是一整行,治理台没法读也没法改(2026-08-24)

**现象**:意图详情页的 SQL 编辑器里,一条模板 SQL 是三四百字符的**一行**,横向滚动才能看完;
`SqlEditor.tsx` 的注释里写着"模板 SQL 是格式化过的多行文本",而这个前提**从来没成立过**。

**根因**:`sqlglot` 的 `tree.sql(dialect="mysql")` 默认不排版,而模板 SQL 的三个产出点
(`template.py` 生成、`rewrite.py::apply_plan` 应用、`executor.py` 的 `sql_executed`)
全都直接用了这个默认值。前端三处显示(D4 编辑器 / D5 引用卡 / Run 面板)本来就是
`whitespace-pre-wrap`,给它多行它就画多行 —— **问题一直在后端,不在前端**。

**修法**:新增 `services/text2sql/sqltext.py` 作为「SQL 长什么样」的唯一出处
(`format_sql()` 文本进文本出 / `render()` 给已有 AST 用),在上述三个产出点调用它,
外加 `seed_s3_demo.py` 灌数时也过一遍(fixture 里仍存 B4 评审定稿的原文,
不让格式化改动搅进那些被人审过的产物文件的 diff)。

**排版只加空白,不动语义**,但走 AST 重生成会带两处等价改写,值得写明白:
`orders o` → `orders AS o`(补 AS)、`!=` → `<>`(MySQL 里完全等价)。
反引号包住的保留字(i16 的 `` `year_month` ``,B4 那轮自修的产物)**原样保留** —— 实测确认过,
丢了这条 SQL 就跑不起来。

**验证**(零 LLM,不动数据库):

- 7 条定稿模板逐条排版后重跑 **7/7 PASS**:B4 的静态校验全过、参数区拆解三区点数与
  评审定稿逐条一致(8/8、8/8、5/5、8/8、13/13、8/8、10/10)、执行闸 + 真库执行全部返回数据
  (4/1/13/10/26/39/10 行),`sql_executed` 全部是多行(6–22 行);
- B8 冻结的 20 个改写计划重放:13 个会生成 SQL 的用例 **13/13 PASS**(多行输出 + 闸和执行都过 +
  `sql_contains`/`sql_not_contains` 断言全中),另外 7 个是拒答用例、本就不生成 SQL;
- 评测集的断言全是**字面值**(`Boomerang Textiles`、`Brisbane`、`2026-01-01` …),
  没有跨关键字的子串,所以排版换行不会误伤它们 —— 这一点是动手前先查过的;
- `uv run pytest` 84 passed、`ruff` / `eslint` / `tsc -b` 全绿。

**刻意没做的两件事**:

1. **人手改过的 SQL 不自动重排**。`PATCH /intents/{id}` 存的是人打进去的原文 ——
   编辑器里的排版是给人看的,不该在保存时被机器改写掉;要不要加一个 Format 按钮是产品决定,
   不在这次修补范围内。
2. **没写数据迁移**。库里当时没有任何 text2sql 数据(需求方刚清空重测),
   新生成/新灌入的都是多行;真有老数据要转,一条 `UPDATE` 配 `format_sql` 就够,不需要 migration。
