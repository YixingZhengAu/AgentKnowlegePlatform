...
2. **选中项是推导出来的,不是一个状态**:`selectedId` 在当前列表里找不到就落到第一条。
   ...
   ★ **唯一的例外是"存过但还没裁决"的那一条**(`pinned`):队列的默认筛选是 `pending`,
   保存一条改动会让它变成 `modified` 而掉出列表,于是上面这条推导会**静默把右侧详情换成
   另一条候选**,紧接着点的通过/驳回就打在了别人身上(S3 的 E1 走查实测踩到,S1 更凶 ——
   那边采纳即发布,误点直接写进正式表)。所以 `save()` 之后把那一条钉在列表顶部:
   **保存不是裁决**;而裁决(通过/驳回)照旧前进到下一条,队列流不变。
