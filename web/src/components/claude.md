...
| `StagingReview.tsx` | **泛型审核台**:筛选/排序/单条/批量(含全选,**只选当前筛选集**,换筛选清空勾选)/键盘流/发布;**动作层可由各域替换**(S1 的采纳即发布);原文面板按域声明的 `originPlacement` 画在下方(默认)或左侧并排可收起(S2)。★ 保存不是裁决 —— 存过未裁决的那一条被钉在列表里,否则右侧详情会静默换人 |
