# server/app/services/document/architect.md

## 定位

文档 RAG(Document RAG) 域(S2)的落点文件夹。本域的 Job 子类、publisher、检索 stage、prompt
...
```
document/
  ingest.py     Job 子类:steps 声明 + 每步实现(产出 staging_items,item_type=chunk)
